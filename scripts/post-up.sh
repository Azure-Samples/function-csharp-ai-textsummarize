...
endpointUrl="https://${AZURE_FUNCTION_APP_NAME}.azurewebsites.net/runtime/webhooks/blobs?functionName=Host.Functions.${function_name}&code=${blobs_extension}"
filter="/blobServices/default/containers/${UNPROCESSED_PDF_CONTAINER_NAME}"

echo "az eventgrid system-topic event-subscription create -n "unprocessed-pdf-topic-subscription" -g "${RESOURCE_GROUP}" --system-topic-name "${UNPROCESSED_PDF_SYSTEM_TOPIC_NAME}" --endpoint-type "webhook" --endpoint "$endpointUrl" --included-event-types "Microsoft.Storage.BlobCreated" --subject-begins-with "$filter""
